      ...
      <main className="section-container py-12">
        <header className="mb-12 border-b border-gray-100 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-black">{t('yourSelection')}</h1>
          </div>
        ...
        {cart.length === 0 ? (
          <div className="py-24 text-center">
            <h2 className="heading-lg text-gray-200 mb-8 italic">{t('emptyCart')}</h2>
          ...
          <div className="grid lg:grid-cols-[1fr_380px] gap-12 items-start">
                ...
                return (
                  <div key={item.id} className="flex gap-6 p-4 border border-gray-100 rounded-2xl bg-white hover:border-gray-200 transition-all group">
                    <div className="relative aspect-[3/4] w-24 sm:w-32 overflow-hidden rounded-xl bg-gray-50 border border-gray-100">
                      <Image src={item.product?.image || '/logo.jpg'} alt={displayName || 'Product'} fill className="object-cover" />
                    </div>
                    ...
                    <div className="flex-1 flex flex-col py-1">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <h2 className="text-sm sm:text-base font-black uppercase tracking-tight text-black leading-tight mb-2">{displayName}</h2>
                          <div className="flex flex-wrap gap-x-4 gap-y-1">
                            {item.selectedSize && <p className="text-[10px] font-bold text-gray-400 uppercase">{t('size')}: <span className="text-black">{item.selectedSize}</span></p>}
                            {item.selectedColor && <p className="text-[10px] font-bold text-gray-400 uppercase">{t('color')}: <span className="text-black">{item.selectedColor}</span></p>}
                          </div>
                        </div>
            ...
            <aside className="border border-gray-100 rounded-2xl bg-gray-50/50 p-8 sticky top-32">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-black mb-8 border-b border-gray-200 pb-4">{t('orderSummary')}</h2>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest text-gray-400">
                  ...
                  <span className="text-black">{shipping === 0 ? t('complimentary') : `€${shipping.toFixed(2)}`}</span>
                </div>
